...
describe('DadosPessoaisTab', () => {
  ...
  it('deve mostrar mensagem de carregamento inicialmente', async () => {
    render(<PersonalDataTab userId="user-1" />);
    expect(screen.getByText('Carregando dados pessoais...')).toBeInTheDocument();

    // Aguardar que o loading termine
    await waitFor(() => {
      expect(screen.queryByText('Carregando dados pessoais...')).not.toBeInTheDocument();
    });
  });
  ...
  it('deve aplicar cor de fonte preta nos campos de entrada', async () => {
    render(<PersonalDataTab userId="user-1" />);
